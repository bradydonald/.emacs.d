...
* Load any lisp files
  ...
  #+BEGIN_SRC emacs-lisp
    ...
    (defun db/add-to-load-path (path feature_toggle)
      "adds a path to the load-path if the it exists and the feature_toggle is on"
      (if feature_toggle
...
** lsp-bridge (Expirmental)
   ...
   #+BEGIN_SRC emacs-lisp
     ...
     (db/add-to-load-path "~/src/lsp-bridge" t)
   #+END_SRC
...
* Flycheck
  ...
  #+BEGIN_SRC emacs-lisp
    ...
    (if (db/add-to-load-path "~/src/flycheck-org-vale" t)
...
* Chat GPT Integration as a Shell
  ...
  #+BEGIN_SRC emacs-lisp
    ...
    (if (db/add-to-load-path "~/.emacs.d/lisp/chatgpt-shell" db/chatgpt-shell)
        (progn 
          (add-to-list 'load-path "~/.emacs.d/lisp/chatgpt-shell")

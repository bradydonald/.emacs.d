...
** Org File Locations
   ...
   #+BEGIN_SRC emacs-lisp
     ...
     (defun db/current-monthly-journal ()
       ...
       (setq current-year (number-to-string (string-to-number (format-time-string "%Y"))))
       (setq current-month (number-to-string (string-to-number (format-time-string "%m"))))
...
** My File Hydra
   ...
   #+BEGIN_SRC emacs-lisp
     (with-eval-after-load 'hydra
       (defhydra find-file-main (:color pink :hint nil :exit t)
         ("i" (db/current-index-file) "Index File")
         ("j" (db/current-monthly-journal) "Monthly Journal File")))
         
     (global-set-key (kbd "C-c f") #'find-file-main/body)     
   #+END_SRC

...
** Org File Locations
   ...
   #+BEGIN_SRC emacs-lisp
     ...
     (defun db/org-agenda-files (&optional keep-reference)
       "Returns the list of agenda files. By default includes all files
     including .org.gpg files but excluding files in the /reference
     directory

       keep-reference, if t will include and return those in the reference directory."
       ...
       (setq tmp-list (append ofiles gfiles))
       (if (not keep-reference)
           (setq tmp-list (cl-delete-if (lambda (r) (string-match-p "\/reference" r)) tmp-list)))
       tmp-list)
  ...
  #+RESULTS:
  : ~/OrgDocuments/personal/index.org

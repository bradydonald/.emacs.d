...
* Vanilla Packages
  ...
  #+BEGIN_SRC emacs-lisp
    ...
    (use-package golden-ratio)      ;; resize windows
     ...
     (defun db/last-download ()
       ...
       (let ((downloads-dir "~/Downloads/"))
         (if (file-directory-p downloads-dir)
             (progn
               (cd downloads-dir)
               (setq files (directory-files downloads-dir nil nil 'nosort))
               (if files
                   (let ((newest-file (car (sort files
                                                 (lambda (a b)
                 ...
                 (message "No files found in %s" downloads-dir)))
           (message "Directory %s does not exist" downloads-dir))))
